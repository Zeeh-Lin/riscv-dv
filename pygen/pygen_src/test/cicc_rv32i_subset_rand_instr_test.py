...
class cicc_rv32i_subset_rand_instr_test(riscv_instr_base_test):
    def __init__(self):
        super().__init__()

    def randomize_cfg(self):
        cfg.instr_cnt = min(cfg.instr_cnt, 120)
        cfg.num_of_sub_program = 0
        cfg.randomize()
        logging.info("cicc_rv32i_subset riscv_instr_gen_config is randomized")
        gen_config_table()
...
cicc_rv32i_subset_rand_test_ins = cicc_rv32i_subset_rand_instr_test()
cicc_rv32i_subset_rand_test_ins.run()
end_time = time.time()
